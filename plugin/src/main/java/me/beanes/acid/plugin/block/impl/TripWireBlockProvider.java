...
public class TripWireBlockProvider extends BlockProvider {
    @Override
    public BoundingBox getBoundsBasedOnState(PlayerData data, int x, int y, int z, WrappedBlockState state) {
        ...
        if (!powered) {
            return new BoundingBox(0.0F, 0.0F, 0.0F, 1.0F, 0.09375F, 1.0F);
        } else if (!attached) {
            return new BoundingBox(0.0F, 0.0F, 0.0F, 1.0F, 0.5F, 1.0F);
        } else {
            return new BoundingBox(0.0F, 0.0625F, 0.0F, 1.0F, 0.15625F, 1.0F);
        }
    }
}
