...
public class LilyPadBlockProvider extends BlockProvider {
    @Override
    public BoundingBox getCollisionBoundingBox(PlayerData data, int x, int y, int z, WrappedBlockState state) {
        return new BoundingBox(x, y, z, x + (double) (0.5F + 0.5F), y + (double) 0.015625F, z + (double) (0.5F + 0.5F));
    }

    @Override
    public BoundingBox getBoundsBasedOnState(PlayerData data, int x, int y, int z, WrappedBlockState state) {
        return new BoundingBox(0.0F, 0.0F, 0.0F, 0.5F + 0.5F, 0.015625F, 0.5F + 0.5F);
    }
}
